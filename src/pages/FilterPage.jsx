...
const filters = [
  {
    ...
    name: 'Place',
    options: [
      { value: 'bistupur', label: 'Bistupur', checked: false },
      { value: 'sakchi', label: 'Sakchi', checked: false },
    ],
  },
  {
    id: 'price',
    name: 'Price Range',
    options: [
      { value: '0-10000', label: '₹0 - ₹10,000', checked: false },
...
export default function FilterData() {
  ...
  const handleFilterChange = (sectionId, optionValue, checked) => {
    setSelectedFilters((prevFilters) => {
      ...
      if (checked) {
        ...
        updatedFilters[sectionId] = updatedFilters[sectionId].filter((value) => value !== optionValue)
      }
  ...
  useEffect(() => {
    ...
    Object.keys(selectedFilters).forEach((filterKey) => {
      if (selectedFilters[filterKey].length > 0) {
        filtered = filtered.filter((property) =>
          selectedFilters[filterKey].some((filterValue) => {
            if (filterKey === 'price') {
              const [min, max] = filterValue.split('-').map(Number);
              return property.price >= min && property.price <= max;
            }
            return property[filterKey].toLowerCase() === filterValue.toLowerCase();
          })
        );
      }
    });
    ...
    switch (selectedSort) {
      ...
      case 'popular':
      default:
        // Assuming 'popular' is the default sorting or no sorting
        break;
    }

    setFilteredProperties(filtered);
  }, [selectedFilters, selectedSort, PropertyList]);
  ...
  return (
    <div className="bg-slate-50">
      <div>
        ...
        <main className="mx-auto px-2 sm:px-3">
          <div className="flex items-baseline justify-between border-b border-gray-200 pb-6">
            <h1 className="text-4xl pt-3 font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-purple-500 to-pink-500">Our Listed Properties</h1>
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
              ...
              <div className="lg:col-span-3">
                <Dashboard properties={filteredProperties} />
              </div>
            </div>
          </section>
        </main>
